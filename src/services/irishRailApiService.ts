...
import {fetchAndParseXML, ArrayOfObjStation, ObjStation} from '../utils/fetchAndParseXML.js'; 
 ...
 */
export async function getAllStations(stationType: StationType | '' = ''): Promise<ArrayOfObjStation> {
...
export async function filterStationsByProximity(latitude: number, longitude: number, maxDistanceKm: number): Promise<any> {
  const stations = await getAllStations();
  return stations.objStation.filter((station:ObjStation)  => {
